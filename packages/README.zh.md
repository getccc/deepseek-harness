...
| [`team/`](team/README.zh.md) | Team Edition Handoff：Control Plane 绑定端点、Runner 账户保管，以及浏览器所导航到的本地地址 |
...
| [`knowledge/`](knowledge/README.zh.md) | 私有知识族：已授权目录、段落检索、会话范围 |
| [`office/`](office/README.zh.md) | 办公交付物选择族：一次对话应生成的文档类型，及其提示词分节与选择器 |
...
| [`guard/`](guard/README.zh.md) | 工具调用守卫：建议性重复调用提醒、`tools/execute` 截止时间强制执行器，以及按组合的工具目录限制 |
