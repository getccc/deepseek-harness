...
`listModels()` 使用当前设备访问 token 读取 `/team/model/catalog`。Control Plane 会先在每个活跃模型资源上评估 `model.discover`，再返回稳定模型引用、显示名与输入模态；模态列表缺失、为空或含有本 Runner 不认识的词的目录条目，会在线路边界被当作畸形响应拒绝。DeepSeek 适配器在独立的 `built-in` 提供方路由上公开这些条目，让成员配置的 `deepseek-official` 路由继续直连。`send()` 使用同一份当前 token 调用 `/team/model/invoke`，网关会在那里独立评估 `model.invoke`、预留配额、解析上游端点与凭据，并把提供方响应流式传回。
...
**运行时不变量：**不发布伴随文件：该传输每次调用读取访问 token 并发送请求体；它不持有状态，发送内容中不含凭据与地址是其构造的请求的属性，测试直接读取该请求。
