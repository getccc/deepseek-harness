...
`@deepseek-ai/dsh-llm-deepseek` is the DeepSeek chat-completions adapter for the harness LLM service: it owns the member-configured `deepseek-official` route and translates DeepSeek's wire format into the harness stream-chunk protocol. The route is registered only while its API-key reference resolves, so a keyless composition advertises no DeepSeek models. With an LLM HTTP transport mounted, it also owns a `built-in` route whose discovery and invocation go through that transport, which keeps the company endpoint, credential, and model authorization on the Control Plane. Endpoint, credentials, catalog, and thinking policy resolve per request, so settings changes apply to the next request.
