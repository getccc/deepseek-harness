 ...
 * registering plugin owns validation, layering, and credential policy. A
 * provider route with a mounted LLM HTTP transport sends the same body
 * through that transport instead, without a key, and reads its model catalog
 * from the transport.
 *
...
export class ChatCompletionsAdapter extends LlmAdapter {
  ...
  constructor(private readonly config: DeepSeekAdapterOptions & {
    ...
    /**
     * The last catalog each transport route listed, owned by the dispatching
     * adapter so it outlives this per-operation instance. Every resolution
     * re-reads the transport, so this serves only the synchronous pricing
     * path, which cannot wait for a listing and is always preceded by one.
     */
    remoteCatalogs: Map<string, readonly DeepSeekCatalogModel[]>
  }) {
    super()
  ...
  override imageRequestPricing(provider: string, model: string): ReturnType<LlmAdapter['imageRequestPricing']> {
    ...
    const pricing = this.config.transport?.(provider) === undefined
      ? connection
      : { ...connection, models: this.config.remoteCatalogs.get(provider) ?? [] }
  ...
  /**
   * The connection snapshot whose catalog is the route's discovery owner's: a
   * direct route's own list, or the catalog a transport lists. A transport
   * route is re-read on every call, so a catalog change on the Control Plane
   * reaches the next request rather than the next restart, and the listing is
   * remembered for {@link imageRequestPricing}. A transport that leaves
   * discovery to the adapter keeps the connection's list.
   */
  ...
  private async * request(
    ...
    const fileConnection = apiKey === undefined
      ? undefined
      : { baseURL: connection.baseURL, apiKey, protocol: connection.protocol }
    const model = connection.models.find(entry => entry.id === options.model)
    ...
    const requestFiles = fileConnection === undefined
      ? undefined
      : new RequestFiles(
    ...
    while (true) {
      ...
      if (!response.ok) {
        ...
        const detail = [providerError?.code, providerError?.type, providerError?.message]
          ...
          .join(' ')
        if (await requestFiles?.retry(detail) === true) continue
