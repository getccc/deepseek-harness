...
模型按提供方分组。composer 菜单只显示模型与推理强度名称。`/model` 弹窗显示提供方名称与目录说明；其中两个内置 DeepSeek 模型的说明使用当前语言，外部提供方说明保持原文。规范的 `built-in` 提供方路由即使缺少可选分类元数据，也使用本地化文案 `内置模型`；其他分类为 `built-in` 的分组使用相同文案，普通分组则使用适配器持有的名称。选择与持久化仍由 provider id 标识。弹窗应用所选模型的默认推理强度；composer 随后可以选择任一已公布的推理强度。适配器没有推理元数据时不显示 Effort 行；不存在任意推理强度输入。
