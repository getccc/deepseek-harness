          ...
          .map((row) => {
            ...
            return {
              ...
              selectionAction: row.broken !== undefined
                ? t('brokenBadge')
                : chatPreset
                  ? t(row.isDefault ? 'chatDefault' : 'noWorkspace')
                  : row.isDefault
          ...
          <section key={trust} className={css.group}>
            ...
            {group.length === 0 ? null : (
              <ul className={css.cards}>
                {group.map(({ row, text, chatPreset, selectionAction }) => (
                  <li
                    key={row.id}
                    className={[
                      css.card,
                      ...
                      !state.showPicker && row.broken === undefined && !chatPreset && !row.isDefault
                  ...
                  >
                    ...
                    <button
                      ...
                      disabled={row.isDefault
                        || (row.broken === undefined && !chatPreset && (!state.showPicker || state.policySaving))}
                      ...
                      onClick={() => {
                        if (row.broken !== undefined || chatPreset) return
