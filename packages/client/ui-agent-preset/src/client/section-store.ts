...
export class AgentPresetSectionController {
   ...
   * Only a `required` row answers: the roster also marks the `none` row that
   * is the chat default, which a blank workspace session cannot take.
   */
  private async confirmEffectiveDefault(showPicker: boolean): Promise<string | undefined> {
    ...
    return state.rows.find(row => row.isDefault && row.workspace === 'required')?.id
  }
