...
interface FakeOptions {
  ...
  /** The `none` preset the roster marks default, as the Host marks its `chatDefault`. */
  chatDefault?: string
}
        ...
        list: () => {
          ...
          return remoteOk({
            presets: [...presets].map(([id, preset]) => ({
              id, trust: preset.trust,
              workspace: preset.workspace ?? 'required',
              isDefault: preset.workspace === 'none' ? id === options.chatDefault : id === effectiveDefault,
...
describe('the default preset', () => {
  ...
  it('syncs the workspace default, never the chat default the roster also marks', async () => {
    const { controller, presets } = harness({ chatDefault: 'chat' })
    const shipped = [...presets]
    presets.clear()
    presets.set('chat', { trust: 'system', content: '- id: persona\n', workspace: 'none' })
    for (const [id, preset] of shipped) presets.set(id, preset)
    const synced: string[] = []
    await controller.load()

    await controller.makeDefault('mine', (id) => {
      synced.push(id)
      return Promise.resolve(undefined)
    })

    // The chat default leads the roster, but a blank workspace session can
    // only take the workspace default the write just moved.
    expect(controller.store.getSnapshot().rows.filter(row => row.isDefault).map(row => row.id))
      .toEqual(['chat', 'mine'])
    expect(synced).toEqual(['mine'])
  })
