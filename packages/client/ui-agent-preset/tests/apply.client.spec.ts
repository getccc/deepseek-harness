...
describe('AgentPresetSeatController reconciliation', () => {
  it('does not retarget a different blank Session after a Settings write', async () => {
    ...
    let current = {
      id: SessionId('first'), blank: true, kind: 'work' as const, projectionValues: { agentPreset: 'standard' },
    }
    ...
    current = {
      id: SessionId('second'), blank: true, kind: 'work' as const, projectionValues: { agentPreset: 'standard' },
    }
  ...
  it('captures no blank chat Session for a Settings write to carry the workspace default to', () => {
    const controller = new AgentPresetSeatController({} as never, () => ({
      id: SessionId('chat'), blank: true, kind: 'chat' as const, projectionValues: { agentPreset: 'chat' },
    }))

    // The settings default is a workspace preset, which the host refuses for
    // a workspace-less session.
