...
describe('the preset list', () => {
  ...
  it('explains and reverses a Host-disabled picker without offering preset changes', () => {
    ...
    // The switch governs workspace presets only: the chat default keeps its
    // own marker and its reason for never being selectable.
    const chat = within(rowFor('chat')).getByRole('button', { name: `${en.chatDefault}: ${en.presetChatName}` })
    expect(chat.getAttribute('aria-disabled')).toBe('true')
    expect(within(rowFor('chat')).getByText(en.noWorkspace)).toBeTruthy()
    expect(within(rowFor('chat')).queryByText(en.selectionOffDefault)).toBeNull()
  })
