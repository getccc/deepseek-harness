...
可选的 `conversation.chat.assistant-identity` slot 为一个轮次的助手活动开头：轮次过程控件显示时由它在自身上方渲染，否则由第一个 Assistant 步骤在其文本上方渲染，因此一个头部位于助手在该轮次所做的一切之上；owner 份额携带轮次、回复的流式状态，以及消息操作行所用的同一个日期感知时钟。没有占位者时不添加任何内容。
