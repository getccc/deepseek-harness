...
The optional `conversation.chat.assistant-identity` slot opens a Turn's assistant activity: the Turn-process control renders it above itself whenever the control is shown, and otherwise the first Assistant step renders it above its text, so one header sits above everything the assistant did in the Turn; the owner share carries the Turn, the reply's streaming status, and the same date-aware clock the message action rows use. With no occupant nothing is added.
