...
- **Ending every browser session at once needs a restart** — `/lock` expires the cookie for the browser that asks, and sessions this Runner issued to other browsers stay valid until the owner credential record is deleted and `dsh` restarts.
