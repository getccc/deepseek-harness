...
- **一次性结束全部浏览器会话需要重启**：`/lock` 只让发起请求的那个浏览器的 cookie 过期；本 Runner 签发给其他浏览器的会话仍然有效，直到删除 owner 凭据记录并重启 `dsh`。
