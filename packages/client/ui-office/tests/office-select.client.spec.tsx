...
const trigger = (): HTMLElement => screen.getByTitle('本次对话要生成的办公交付物')
...
describe('OfficeSelect', () => {
  ...
  it('records the visualization kind', async () => {
    ...
    await act(async () => { fireEvent.click(trigger()) })
    await act(async () => { fireEvent.click(screen.getByRole('menuitem', { name: '可视化' })) })
    expect(apply).toHaveBeenCalledWith('chart')
  })
