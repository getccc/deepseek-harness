...
describe('command launcher chrome and control seats', () => {
  it('renders the command launcher and dispatches every empty control seat', () => {
    ...
    expect([...new Set(slotCalls.map(c => c.key))]).toEqual([
      ...
      'conversation.input.right', 'conversation.input.model', 'conversation.input.voice',
  ...
  it('a chat session drops the permission and plan seats; the launcher, both zones, the model seat, and the context meter stay', () => {
    ...
    const chat = bench({ kind: 'chat', contextPressure, ...entries })
    // The kind is the owner's word: both mode seats have registered entries
    // and still neither is dispatched for a tool-less session.
    expect(chat.view.queryByTestId('permission-entry')).toBeNull()
    expect(chat.view.queryByTestId('plan-entry')).toBeNull()
    expect(chat.slotCalls.map(call => call.key)).not.toContain('conversation.input.permission')
    expect(chat.slotCalls.map(call => call.key)).not.toContain('conversation.input.plan')
    expect(chat.view.getByLabelText('添加文件或调用指令')).toBeTruthy()
    ...
    const work = bench({ kind: 'work', contextPressure, ...entries })
    expect(work.view.getByTestId('permission-entry')).toBeTruthy()
