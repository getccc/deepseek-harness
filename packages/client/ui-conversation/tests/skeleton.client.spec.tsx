...
function mount(
  ...
  const renderSlot = ((key: string, owner: object, opts?: { only?: string; fallback?: ReactNode }) => {
    ...
    // Unfilled here, like a composition with no account: the shell's own line.
...
describe('Hero chrome', () => {
  it('offers the headline as a slot, and says its own line when nobody fills it', () => {
    // The occupant is where a greeting by name comes from, so the slot has to
    // carry the headline's own class: an occupant that styled itself would
    // drift from the line it replaces.
    const renderSlot = vi.fn<HeroShellProps['renderSlot']>((_name, _owner, opts) => opts?.fallback ?? null)
    const view = render(<HeroShell t={makeTranslate(en, commonEn)} renderSlot={renderSlot} />)
    expect(view.getByText('What is the plan today?')).toBeTruthy()
    expect(renderSlot).toHaveBeenCalledOnce()
    expect(renderSlot.mock.calls[0]?.[0]).toBe('conversation.hero.headline')
    const owner = renderSlot.mock.calls[0]?.[1]
    if (owner === undefined || !('className' in owner)) {
      throw new Error('hero headline owner must provide className')
    }
    expect(owner.className).toBeTypeOf('string')
  })
})
...
describe('ConversationRoot resident composer', () => {
  ...
  it('keeps a rejected first prompt engaging instead of returning to the Hero', () => {
    ...
    expect(b.view.queryByText('今天有什么计划？')).toBeNull()
  })
