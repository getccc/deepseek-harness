...
import { IconGlobeOutline14, IconGoalOutline16 } from '@deepseek-ai/dsh-client-ui-primitives'
...
describe('candidates', () => {
  ...
  describe('menu presentation (design doc for #3567)', () => {
    ...
    it('the per-session web switch is a built-in row: localized face, closing Commands, and the typed 联网 spelling', async () => {
      const commands: CommandDescriptor[] = [
        ...SHIPPED,
        { definitionId: CommandDefinitionId('@deepseek-ai/dsh-tool-web'), name: 'web', description: 'Turn web search and page fetching on or off for this session', input: { hint: '[on|off]' } },
      ]
      const { source, warm, executeCalls } = await bench({ commands: () => Promise.resolve({ commands }) })
      await warm(proj('s1'))
      const rows = await source.candidates(proj('s1'), req(''))
      expect(rows.at(-1)).toEqual({
        name: 'web',
        label: 'command:label.web',
        description: 'command:description.web',
        icon: IconGlobeOutline14,
        hint: '[on|off]',
        section: 'command:section.commands',
      })
      const space = source.matchSpace!(proj('s1'), '/联网')
      if (space === undefined || space === 'handled' || !('claim' in space)) throw new Error('expected the web claim')
      expect(space.claim.name).toBe('web')
      await space.claim.submit('off', new Context(), [])
      expect(executeCalls).toEqual([{ sessionId: sid('s1'), line: '/web off', images: [] }])
    })
  })
})
