...
贡献项是客户端自有命令——与宿主命令同名会明确报错。`popupSelect` 的行在回车或点击时生效并关闭弹窗；`popupMultiSelect` 的行改为勾选，勾选在搜索将其隐藏后依然保留，整组勾选经底栏控件或 ⌘/Ctrl+回车一次性生效。勾选本身不生效，成员可以离开多选弹窗而不改变任何东西。标记为 `exclusive` 的行是“全部”行：勾选它会清空逐项勾选，勾选任一逐项行也会清空它。装饰为**已存在的**宿主命令添加裸调用弹窗：宿主命令保留其目录行、参数声明与生命周期记账，被装饰的名字在会话目录中无宿主行时永不触发。菜单查询按顺序且不区分大小写地模糊匹配命令名的子序列；前缀排名最高。
