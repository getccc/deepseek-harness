...
Outside a Sidebar tab, a surface that already holds a complete document declares the root-scoped `document.view` chain as a child of its own entry and renders it with `renderSlotChain('document.view', { fileName, content }, { fallback })`, where `content` is `{ kind: 'text', text }` or `{ kind: 'bytes', data }`. Entries claim a document through their selector: the Markdown entry claims text whose file name ends in `.md` or `.markdown`, and the PDF entry claims bytes whose file name ends in `.pdf`, both matched with the same suffix rules as the registry. The claiming entry draws the document settled and from its first page, with no tab, store, or reload; with no claiming entry the owner's fallback renders. The chain exists only while some surface declares it, so these entries wait for it through `ctx.slots.inject`. The knowledge panel's document drawer is that surface today.
...
- **The complete-document view draws Markdown and PDF only.** Code, HTML, images, and the Office renderers register only in the tab slot, so a `document.view` owner draws those documents with its own fallback.
