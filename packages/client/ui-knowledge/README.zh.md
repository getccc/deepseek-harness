...
控件读取本次对话的范围：未选择时以 composer 自身的墨色显示`知识库`，选择之后改用业务色，显示全部集合或单个所选知识库的名称，选了多个时显示数量（`2 个知识库`），因为拼接的名称会把模型和发送控件挤到第二行。收窄到文档的对话——从知识库检索结果打开的那种——显示为它的知识库加一个数量（`临港知识库 · 3 篇文档`），这就是日志对它所记录的全部；点击该知识库那一行会把对话放宽回整个知识库，而不是关闭知识库。它的无障碍名称始终列出每个所选知识库。composer 控件行窄于 460px 时它只显示图标，与权限控件一致。点击它打开的是与指令相同的那套选择。
...
本插件自行挂载仅 Team 的 `knowledge` 远程命名空间，而不经由共享的 Client 装配，因为只有 Team 宿主提供它——否则没有私有知识的构建会多出一个每次调用都失败的命名空间。这次挂载与其他贡献一样是一个 effect，两个界面都停靠在它提供的 `remote.knowledge` 服务上，因此它们不会早于所调用的命名空间出现，也会先于它消失。`src/client/scope.ts` 拥有点击与记录范围之间的全部映射：`optionsOf` 依据 `knowledge.scope` 绘制选择器的行，`choiceOf` 把一组勾选读作 `off`、`all` 或具名选择，`toggleScope` 则把菜单上的一次点击按当前生效的选择读成同样的东西，最后由 `knowledge.choose` 记录。`/knowledge` 是注册在 `ctx.commandUi` 上的 `popupMultiSelect` 贡献项，其互斥首行就是「全部」选择背后的外壳机制；composer 控件占据 `conversation.input.left` 区域，经标准套件的 `useProjection` 读取宿主计算的 `knowledge` 投影，因此两个界面都不保存选择的副本；该控件还经标准套件的 `useSessions` 读取 Session 的 `kind`，对聊天 Session 不渲染任何内容，因为在聊天里知识库不是成员挑选的东西。菜单上的一次点击，依据的是它同一次目录应答里的范围，而不是它可能还没看到的投影。文档卡片占据 composer 卡片顶部的 `conversation.input.context`，读取同一个投影：范围收窄到文档时，它们以附件文件卡片的形态为每份文档显示一张卡片——文件类型、日志记录的标题和所属知识库——且对任何 Session 类型都显示，因为知识面板会打开一个收窄到某份文档的聊天，这是聊天携带知识的唯一途径。卡片的 × 通过 `knowledge.chooseDocuments` 带上其余标题移除这份文档，最后一份被移除时记录 `off`，因为成员移除的是他唯一的文档，而不是要它所在的整个知识库。失败文案按错误面策略保持英文，并带上远程自己的 code。
