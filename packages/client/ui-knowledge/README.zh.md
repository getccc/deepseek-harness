...
控件读取本次对话的范围：未选择时以 composer 自身的墨色显示`知识库`，选择之后改用业务色，显示全部集合或单个所选知识库的名称，选了多个时显示数量（`2 个知识库`），因为拼接的名称会把模型和发送控件挤到第二行。它的无障碍名称始终列出每个所选知识库。composer 控件行窄于 460px 时它只显示图标，与权限控件一致。点击它打开的是与指令相同的那套选择。
