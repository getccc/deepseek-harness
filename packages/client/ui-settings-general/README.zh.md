...
用户通常通过侧边栏底部的 Settings 控件进入外壳；功能可以占用可选的单值 `settings.launcher` Slot 来替换该入口，同时接收同一个 `openSettings()` 动作与侧栏宽度状态。功能插件通过本外壳所投影的 slot 账本贡献自己的页面与引导步骤。外壳渲染模态面板、由 `settings.section` 条目构建的导航，以及每次只挂载一个的引导步骤。
