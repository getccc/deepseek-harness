...
const summary = (id: string, updatedAt: number, overrides: Partial<SessionSummary> = {}): SessionSummary => ({
  id: sid(id), displayTitle: id, kind: 'work', running: false, blank: false, updatedAt, ...overrides,
})
...
describe('WorkspaceBrowser', () => {
  ...
  it('keeps chat Sessions out of the Ungrouped and flat-list manual orders', () => {
    const b = mount({
      useSessions: hook(sessionState([summary('loose', 10), summary('conversation', 20, { kind: 'chat' })])),
    })
    act(() => { b.store.actions.setGroupExpanded(UNGROUPED_KEY, true) })
    expect(screen.getAllByRole('treeitem').filter(row => row.getAttribute('aria-expanded') === null)
      .map(row => row.querySelector('[class*="title"]')?.textContent)).toEqual(['loose'])
    expect(b.store.getSnapshot()).toMatchObject({
      orderBy: 'manual',
      sessionOrderByAccount: { [UNGROUPED_KEY]: ['loose'], [FLAT_SESSION_ORDER_KEY]: ['loose'] },
    })
  })
