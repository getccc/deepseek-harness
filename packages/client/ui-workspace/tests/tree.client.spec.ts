...
import {
  deriveFlat, deriveGroups, deriveRecent, deriveSearchResults, orderByRecency, owningGroupKey,
...
describe('deriveGroups', () => {
  ...
  it('projects pending-interaction state into grouped and flat rows', () => {
    ...
    expect(deriveFlat(sessions, visibleSessionIds(sessions, noArchive, 'work'), attention)[0])
  ...
  it('projects the completion reminder into session and search rows (absent = false)', () => {
    ...
    expect(deriveFlat(sessions, visibleSessionIds(sessions, noArchive, 'work'), noAttention)
...
describe('deriveFlat', () => {
  ...
  it('hides subagent-origin rows but keeps ordinary forks', () => {
    ...
    const ids = visibleSessionIds({ ...list(parent, fork, subagent), current: subagent.id }, noArchive, 'work')
  ...
  it('tolerates ids whose summary has not landed yet', () => {
    ...
    expect(visibleSessionIds(partial, noArchive, 'work')).toEqual([sid('present')])
  })
  ...
  it('shows only the current blank session and excludes blanks from search', () => {
    ...
    const rows = deriveFlat(sessions, visibleSessionIds(sessions, noArchive, 'work'), noAttention)
  ...
  it('hides archived sessions in flat mode', () => {
    ...
    expect(visibleSessionIds(list(kept, gone), archived('gone'), 'work')).toEqual([kept.id])
  })
})
...
describe('deriveRecent', () => {
  it('admits rows by kind, newest-first with the id tiebreak, and every kind under all', () => {
    ...
    // The work filter selects the same members the flat list orders.
    expect(visibleSessionIds(sessions, noArchive, 'work')).toEqual([sid('work-old'), sid('work-new')])
  })
...
describe('chat sessions outside the Workspace tree', () => {
  it('never enter Ungrouped or the flat list, while search still matches them', () => {
    ...
    expect(visibleSessionIds(sessions, noArchive, 'work')).toEqual([work.id])
