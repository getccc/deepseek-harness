...
export function WorkspaceBrowser({
  ...
  const currentBlank = list.current !== undefined && list.byId[list.current]?.blank === true
    ? list.current
    : undefined
  // Chat Sessions belong to the Recent list, never to Ungrouped or the flat list.
  const ungroupedMemberIds = useMemo(() => {
    const accounted = new Set(workspaces.flatMap(workspace => workspace.sessionIds))
    return list.ids.filter(id => list.byId[id]?.kind === 'work' && !accounted.has(id))
  }, [list, workspaces])
  const flatMemberIds = useMemo(
    () => visibleSessionIds(list, archivedSessionIds, 'work'),
