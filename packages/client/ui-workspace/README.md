...
Below the Workspace tree, the **Recent** list shows Sessions newest-first under a persisted kind filter: **Chat** (the default) lists conversations started without a Workspace, **Work** lists Workspace Sessions, and **All** lists both. A chat Session never appears in the Workspace tree or its flat list; the selected blank chat shows as a provisional **New chat** row until its first prompt. `ctx.uiWorkspace.startChat()` reuses the pristine blank chat Session when one exists and otherwise creates one by naming no Workspace, then opens it like New Session does.
