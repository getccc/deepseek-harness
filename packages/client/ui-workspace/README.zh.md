...
Workspace 树下方的**最近**列表按最新优先展示 Session，不受下文视图选项影响，并受一个持久化的类别过滤器控制：**聊天**（默认）列出不带 Workspace 开始的对话，**工作**列出 Workspace Session，**全部**两者都列。聊天 Session 绝不会出现在 Workspace 树或其单列表中；当前选中的空白聊天在首条提示词落地前显示为一条临时的**新对话**行。`ctx.uiWorkspace.startChat()` 在存在 pristine 的空白聊天 Session 时复用它，否则以不命名 Workspace 的方式新建一个，然后像 New Session 一样打开。
