---
description: "Control Plane 自己的页面：登录、确认一台电脑，以及成员、角色与设备的管理页面。"
...
`dsh-team-shell` 提供成员与管理员在 Control Plane 上使用的页面：登录、确认一台请求连接的电脑，以及成员、角色和设备的管理页面。每一次写入都被授权两次——会话说明是谁在问，访问控制说明他们是否可以——而每一次管理行为都留下一条指名执行者的审计记录。这些页面是服务端渲染且不含脚本的，因为成员是在其他任何东西加载之前到达其中第一个页面的。
...
一个会话、一个指名同一 authority 的 `Origin`，以及一个由会话导出的 CSRF Token。该 Token 是导出的而不是存储的，因此没有第二份记录需要保持同步；它又是一个不同于 Cookie 的值，因此单是携带 Cookie 并不足以提交一个表单。
...
- **不能编辑授权** —— 角色可以创建和绑定，但角色内部的授权是通过访问控制服务设置的，而不是通过某个页面。
- **没有 Enrollment Link** —— 管理员添加一个成员，而设置该成员的首个密码仍然需要另一条路径。
- **页面无样式且不分页** —— 很长的成员或设备列表会被整体渲染，这对本版本所面向的规模是可以的，但不会一直如此。
