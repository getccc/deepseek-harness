...
describe('the local entry', () => {
  it('sends a locked browser to the local login page, never to the Control Plane', async () => {
    ...
    expect(page.body).toMatch(/<span class="brand-mark" aria-hidden="true"><img src="data:image\/webp;base64,[^"]+" alt="" \/><\/span>/)
    expect(page.body).toContain('<span class="brand-name">WeWork</span>')
