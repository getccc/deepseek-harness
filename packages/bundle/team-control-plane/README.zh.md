...
该 profile 只组合本 bundle，并监听 `127.0.0.1:3095`。普通成员不会浏览此端口；管理员访问 `/team/admin/`，Runner 则调用设备、模型、知识、BI 与网页搜索端点。
...
BI 数据源接受同样三个没有默认值的配置项：webi 提供方上的 `sourceCode`、`baseUrl` 和 `credentialRef`，出于同样的理由在加载期限制并拒绝。凭据必须解析为专为本部署创建的 webi 组织管理员的个人访问令牌，而不是某个人的：一个项目授权同时覆盖项目的私有空间，能到达范围更小的令牌会让授权悄悄地比管理员所做的更窄。webi 通过部署给 `baseUrl` 的网络地址访问；令牌永远不离开本进程，Runner 请求至多携带一个项目引用和一个关键词。
...
这些存储默认使用预写日志（WAL）。已提交的事务在检查点把它写入数据库文件之前，可能只存在于 `<file>-wal` 中，因此备份存储要用 `sqlite3 <file> ".backup <target>"`，而不是只复制数据库文件。带有此默认值的构建首次启动时，会把已有文件切换为 WAL。持有写锁的 `sqlite3` 会话会让请求最多等待 `busyTimeoutMs`（默认 1 秒）后失败，等待期间整个进程也随之阻塞。若文件系统无法承载 WAL 的共享内存文件，需要在每个存储的配置行上把 `journalMode` 设为回滚日志模式。
