...
`dsh-tool-office` 是模型所见的一次对话办公选择的全部：一个命名要生成文档类型的提示词分节，另附该类型已配置的技能，对 PowerPoint 还附公司模版。它不注册工具：模型用 Team 部署安装的 univer 办公工具构建文档、导出文件，并用 `present` 声明它，从而列入交付物行与 Sidebar。该分节折叠 `office/kind` 会话日志，而编辑器芯片把同一折叠作为 `office` 投影读取，因此选择器与模型绝不冲突。
...
选择为 `none` 时没有任何内容。否则是一段约 150 token 的固定段落；已配置的技能名或模版路径是其中一部分，且不随对话增长。比分节本身大小更重要的是减少工具调用：构建回合里的每个模型请求都会重发整个上下文。
...
- **位图需要打过补丁的 univer 导出** —— `dsh-univer-office` 0.2.14 把导入与插入的 PNG、JPEG 图片存为其导出器无法读取的资源，因此从公司模版导入的演示文稿在插件于导出前把它们内联之前，`univer_export` 都会失败（[办公构建只用少量工具调用](../../../.agents/notes/implemented/feature/2026-09-15-office-builds-take-few-tool-calls.zh.md)）。
