...
|---|---|---|
| `pptSkill` | — | The session-catalog skill every `ppt` deliverable starts by loading; it owns the template workflow, and a configured path below still names the template file |
...
##### With the PowerPoint kind chosen and a skill and template path configured
