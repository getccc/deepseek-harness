...
`office/` 组让成员在编辑器里指定一次对话应生成哪种办公交付物——Word 文档、Excel 工作簿、PowerPoint、公司的 Welinkin PowerPoint 模版，或可交互图表——并把该选择折叠进模型读取的提示词分节。它存在的原因是：在成员点明格式之前，“做个演示”和“做个表格”是同一个请求，而格式是逐对话的决定，必须在恢复、分叉与第二个浏览器之间保持。该组拥有词汇：一次对话可选的 `OfficeKind`、记录它的 `office/kind` 会话事件，以及恢复它的折叠。它不拥有任何文档引擎——生成文件是 univer office 工具的职责，绘制图表是 Web 界面的职责；本组只命名目标，并在 Welinkin 类型时指向随附的模版。
