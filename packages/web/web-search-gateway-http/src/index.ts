/**
 * The Control Plane's Runner-facing web search route: verify the device
 ...
 * refusal. A request cannot say who is asking or which provider answers.
...
export function apply(ctx: Context, config: Config): void {
  ...
  const search: WebRoute = {
    ...
    handler: async (req, res) => {
      ...
      // The version is decided before any other field is read and before the
      // token is verified: decoding a body under a grammar its sender did not
      // mean is how a version check stops being one, and sending an
      // out-of-date Runner to sign in again cannot help it.
      ...
      const principal: Principal = { orgId: claims.orgId, principalId: claims.principalId, deviceId: claims.deviceId }
      await govern(principal.orgId)
      // Asked fresh on every call, so a revoked grant lands on this search.
