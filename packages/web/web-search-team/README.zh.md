...
账户客户端会刷新设备访问令牌，缓存的副本必然是过期的那份。这也是为什么没有任何搜索凭据来到这里：本进程持有的唯一东西是"谁已登录"的短期证明。提供方始终报告自己可用；该成员能否搜索是控制面的决定，每次调用都重新做出。它的 `permitted()` 向 `POST /team/web/access` 询问该决定，`dsh-tool-web` 每个会话读取一次，只向可以使用的成员提供联网开关；凡不是明确的「是」，包括未绑定的电脑或无法抵达的控制面，都是「否」。
