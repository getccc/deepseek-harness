...
| [`bi-controller/`](bi-controller/README.md) | Team-only: reads the BI projects a member may analyze and records one Session's project choice as `bi/scope` for the composer control. | `ctx.biController` / `ctx.remote.bi` |
