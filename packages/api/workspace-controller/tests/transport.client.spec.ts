...
/**
 * Boot the Gateway client with the Workspace command answers and the Host's
 * path-open confirmation registered, so `remote.workspace` and `remote.session` are provided.
 */
async function gatewayClient(mock: RemoteMock, start: () => Promise<TestClient>): Promise<{ remote: ClientRemote; client: TestClient }> {
  mock.load(workspaceWorld).unary('session/openWorkspacePath', ok({ opened: true }))
...
describe('WorkspaceController', () => {
  it('publishes the model source and exposes successful Workspace commands', async ({ mock, start }) => {
    ...
    const controller = new WorkspaceController(client.ctx, model, remote.session)
    ...
    expect(mock.log.requests('session/openWorkspacePath')).toEqual([{ path: '/work/one/report.docx' }])
  ...
  it('maps generated business failures to the command facade errors', async ({ mock, start }) => {
    const { remote, client } = await gatewayClient(mock, start)
    const controller = new WorkspaceController(client.ctx, new ClientWorkspaceModel(remote.workspace), remote.session)
    // The Host's own words reach the dialog that shows a refused open.
    mock.remote.session.openWorkspacePath.mockResolvedValueOnce(err(new RemoteError(
      'gateway/internal', 'path open failed: xdg-open is not available', {},
    )))
