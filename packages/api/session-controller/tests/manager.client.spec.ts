...
describe('remaining branches', () => {
  ...
  it('create passes cwd and a preallocated id, folds transport throws, and deduplicates the echo', async ({ mock, remote }) => {
    ...
    remote.session.create.mockImplementation(request => Promise.resolve(ok({
      sessionId: S1, ...request.cwd === undefined ? {} : { cwd: '/tmp/w' },
    })))
    const manager = makeManager(mock, remote)
  ...
  it('create naming no location sends no cwd and lands a pristine placeholder that owns none', async ({ mock, remote }) => {
    // The host composes a Session without a location and echoes no cwd.
    remote.session.create.mockResolvedValue(ok({ sessionId: S1 }))
    const manager = makeManager(mock, remote)
    await manager.create()
    expect(remote.session.create).toHaveBeenCalledExactlyOnceWith({})
    expect(manager.getListSnapshot().items).toEqual([
      expect.objectContaining({ sessionId: S1, blank: true, pristine: true }),
    ])
