...
import { JUMP_PAGE_MESSAGES, turnOpenedInside, type Session } from '../src/client/sessions/session.ts'
...
describe('opening inside a turn', () => {
  ...
  it('pages back to the turn/start of a cut turn and loads no older turn', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    mock.stream(FOLLOW, followScript(history(slice(20, 30), true)))
    mock.remote.session.page.mockImplementation(pageRule(request => request.beforeSeq === 20
      ? history(slice(12, 20), true)
  ...
  it('pages nothing when the window opens at a turn boundary', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    mock.stream(FOLLOW, followScript(history(slice(6, 30), true)))
    await session.open()
    expect(mock.log.requests(PAGE)).toEqual([])
    expect(eventSeqs(session)[0]).toBe(6)
  })

  it('stops on a page that makes no progress', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    mock.stream(FOLLOW, followScript(history(slice(20, 30), true)))
  ...
  it('fails soft on a folded carrier page failure and keeps the opened window', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    mock.stream(FOLLOW, followScript(history(slice(20, 30), true)))
    mock.remote.session.page.mockImplementation(() => Promise.reject(new Error('page wire down')))
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined)
  ...
  it('fails soft on a local paging throw with console.error and keeps the opened window', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    mock.stream(FOLLOW, followScript(history(slice(20, 30), true)))
    const errorSpy = vi.spyOn(console, 'error').mockImplementation(() => undefined)
    const prepend = vi.spyOn(SessionEventStream.prototype, 'prepend').mockRejectedValueOnce(new Error('local fault'))
    try {
      await session.open()
      expect(errorSpy).toHaveBeenCalledWith('[session-controller] opening turn load failed:', expect.any(Error))
      expect(eventSeqs(session)[0]).toBe(20)
      expect(session.getSnapshot()).toMatchObject({ openState: 'open', loadingOlder: false })
    } finally {
      prepend.mockRestore()
  ...
  it('stops when the event stream generation moves while a page is in flight', async ({ mock, start }) => {
    const session = await sessionBench(mock, start, SID)
    // The first opening cuts turn 1; the rebuilt generation opens at its turn/start.
    mock.stream(FOLLOW, followScript(history(slice(20, 30), true)))
    const gate = Promise.withResolvers<RemoteResult<SessionPage>>()
    mock.remote.session.page.mockReturnValueOnce(gate.promise)
    ...
    mock.stream(FOLLOW, followScript(history(slice(6, 30), true)))
    const rebuilt = session.resync()
    gate.resolve(history(slice(12, 20), true))
    await opened
    await rebuilt
    expect(mock.log.requests(PAGE)).toHaveLength(1)
    expect(mock.log.requests(FOLLOW)).toHaveLength(2)
