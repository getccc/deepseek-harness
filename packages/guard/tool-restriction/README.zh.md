...
至少要声明一个列表：什么都不遮蔽的行通不过校验，缺省的列表也绝不会被当成空列表。两个列表同时声明时取交集。没有任何已注册全局工具使用的名字会在该行挂载时失败，对 preset 而言就是第一次组合它的会话。生成的[配置目录](../../../docs/config-catalog.zh.md#deepseek-aidsh-tool-restriction)记录每个可接受的值。
