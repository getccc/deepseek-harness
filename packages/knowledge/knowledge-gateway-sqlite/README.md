...
- **Authorization iterates** — a directory and an `all`-mode search evaluate once per knowledge base; a document listing evaluates once, for the one it names. This is acceptable for a full-listing deployment; a bulk access-control operation would have to preserve per-resource evaluation and policy-revision semantics rather than authorizing once for several.
