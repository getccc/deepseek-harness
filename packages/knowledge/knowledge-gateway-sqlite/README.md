...
- **Authorization iterates** — a directory and an `all`-mode search evaluate once per knowledge base; a document listing or read evaluates once, for the one knowledge base it names.
- **A content read holds the whole file** — bytes pass through this process in memory, bounded by `defaultMaxDocumentBytes`, with no streaming and no cache. A deployment that wants larger documents raises the bound and pays for it in memory per concurrent read. This is acceptable for a full-listing deployment; a bulk access-control operation would have to preserve per-resource evaluation and policy-revision semantics rather than authorizing once for several.
