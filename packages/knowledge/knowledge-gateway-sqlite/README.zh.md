...
- **授权是逐个进行的** —— 目录和 `all` 模式检索每个知识库判定一次；文档列表或阅读只为它命名的那一个知识库判定一次。
- **一次内容读取会持有整个文件** —— 字节在本进程内存中经过，由 `defaultMaxDocumentBytes` 限定，不做流式传输也不做缓存。想要更大文档的部署可以提高这个上界，代价是每个并发读取所占的内存。这对完整列表部署可接受；批量访问控制操作必须保留逐资源判定和策略修订语义，而不能一次授权后执行多个。
...
- **schema 版本会把既有文件搁死** —— `SCHEMA_VERSION` 单调递增、两个方向都拒绝，且不带迁移代码。schema 4 新增了 `knowledge_base.upstream_created_at`，因此持有 schema 3 文件的部署必须先加上这一可空列并设置 `PRAGMA user_version = 4`，本次构建才会打开它；该列会在下一次成功列举时回填，而文件里的授权与管理员开关正是「删掉重建」不可取的原因。
...
**运行时不变量：**不发布伴随文件：该 provider 不发布事件流；返回的每个段落都来自本请求授权的知识库，这一点在 `search` 内强制执行，并由本包测试断言。
