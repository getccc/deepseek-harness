...
`POST /team/knowledge/catalog` takes `protocolVersion` alone and answers the principal's authorized directory. `POST /team/knowledge/documents` adds `ref` and an optional `page` and `pageSize`. `POST /team/knowledge/document` adds `docRef` and an optional `maxBytes`. `POST /team/knowledge/search` adds `query`, `scope`, an optional `maxResults`, and an optional `maxDocuments` that asks for a document ranking; a deployment that predates `maxDocuments` ignores it and ranks passages, which answers fewer documents and never a wider scope, so no version marks it. No body has a field for an organization, a principal, a device, a source, an address, a tenant, a credential, or an upstream id — a Runner names what it wants, and the Control Plane resolves and authorizes the rest.

Each added route raised the current protocol version and left the minimum alone. The version travels per request rather than per Runner: a request declares the version its own facts were settled at, so the range holds in both directions. A Runner built before a route keeps every route it knew and has nothing that calls the newer one; a Runner updated ahead of its deployment keeps the routes that deployment serves — its directory and a whole-knowledge-base search — and is refused only on the newer request. A search narrowed to documents declares the document version for the same reason a route does: a deployment that predates narrowing would answer with the whole knowledge base, which is wider than the member asked for.
