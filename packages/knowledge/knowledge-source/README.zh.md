...
`dsh-knowledge-source`（`ctx.knowledgeSource`）是受治理的知识 gateway 与上游知识产品对话所经的接缝。它的各个操作——列出一个源、列出并读取某个知识库中的文档、在其知识库的显式集合中搜索——都以上游的术语表达：用上游 id 而非 `KnowledgeRef`，因为两者之间的映射是目录的职责。它只挂载在 Control Plane，绝不在 Runner 中。没有任何操作接受 URL、调用方自选的 header 或任意上游路径，因此前面的 gateway 不可能被诱导执行权限目录未治理的操作。
...
`listDocuments` 收到一个已授权的上游 id 和一个页码，并且还欠 gateway 一个承诺：它回答的每个 `upstreamDocId` 都装得进受治理的文档引用——不超过 `KNOWLEDGE_DOC_ID_MAX_LENGTH`、落在引用字符表内——任何其他情况的行都以 `upstream-invalid` 拒绝。正因如此，gateway 才能不加第二道判断就用它铸出引用。读不懂的总数报告为缺失而不是零，因为读者不能被告知列表在它并未结束的地方结束。
...
- **每次读取内容会读两次记录** —— `describeDocument` 回答文档位于何处，供 gateway 做授权判定；`fetchDocument` 再读一次记录，以取得它据以判断的文件名和大小。两者各自都是对的，且都不信任另一方的副本；在意第二次调用的部署只能改为跨接缝传递元数据。
...
- **没有增量列表** —— `list()` 返回数据源持有的全部知识库，没有分页也没有变更游标；只有文档列表分页。持有成千上万个知识库的数据源会需要它。
