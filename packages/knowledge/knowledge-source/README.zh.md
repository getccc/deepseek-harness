...
`dsh-knowledge-source`（`ctx.knowledgeSource`）是受治理的知识 gateway 与上游知识产品对话所经的接缝。它的各个操作——列出一个源持有什么、列出某个知识库中的文档、在其知识库的显式集合中搜索——都以上游的术语表达：用上游 id 而非 `KnowledgeRef`，因为两者之间的映射是目录的职责。它只挂载在 Control Plane，绝不在 Runner 中。没有任何操作接受 URL、调用方自选的 header 或任意上游路径，因此前面的 gateway 不可能被诱导执行权限目录未治理的操作。
...
- **没有文档内容** —— 数据源可以被列出、分页和检索；没有任何东西返回某一份文档所持有的内容。该操作随新增它的那个阶段一起到来。
