...
`KnowledgeCatalogEntry` 把 `adminEnabled` 与 `remotePresent` 分开，因为它们是不同的事实：前者是同步不得覆盖的策略选择，后者是同步发现的情况。`effectiveEnabled` 是二者与受治理资源自身状态的合取，因此中断的写入读作已停用，而不是可用。携带 `embeddingModelId` 是因为跨不共享同一模型的知识库的检索会被拒绝，而管理员否则看不到原因。
...
- **消失条目没有清理** —— 数据源不再列出的条目会被停用并保留，因为 `deleteResource` 还会删除所有指名它的授权。移除它是一个目前没有任何地方提供的独立显式操作。
