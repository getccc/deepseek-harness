...
`dsh-knowledge-gateway`（`ctx.knowledgeGateway`）是位于成员 Runner 与知识源之间的 Control Plane 服务——唯一决定某个主体可触达哪些知识库的一方。它由一个所有者服务两类受众：管理员读取并维护持久目录，Runner 读取经授权的目录并搜索它。两者都经过这里，因为目录与授权决定必须一致；一个展示了搜索会拒绝的知识库的目录，比二者单独存在更糟。导入它以编写 gateway 提供方，或消费一个。
