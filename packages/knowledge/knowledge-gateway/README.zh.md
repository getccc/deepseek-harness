...
`dsh-knowledge-gateway`（`ctx.knowledgeGateway`）是位于成员 Runner 与知识源之间的 Control Plane 服务——唯一决定某个主体可触达哪些知识库的一方。它由一个所有者服务两类受众：管理员读取并维护持久目录，Runner 读取经授权的目录、列出某个知识库中的文档并搜索它。两者都经过这里，因为目录与授权决定必须一致；一个展示了搜索会拒绝的知识库的目录，比二者单独存在更糟。导入它以编写 gateway 提供方，或消费一个。
...
- **没有文档内容** —— 目录、其中的文档和检索就是面向成员的全部接口面；没有任何东西返回某一份文档所持有的内容。
- **文档列表按检索判定** —— 准许它的是 `knowledge.search`，这是产品决策，而不是因为两个操作相同。想把两者分开的部署需要授予 `knowledge.read` 并修改提供方的动作常量（[Agent Note](../../../.agents/notes/proposed/feature/2026-09-16-member-knowledge-browsing-and-retrieval.zh.md)）。
