---
description: "受治理知识网关接缝（ctx.knowledgeGateway）：管理员维护的持久化目录，以及 Runner 访问的已授权目录、文档列表与检索。"
...
`dsh-knowledge-gateway`（`ctx.knowledgeGateway`）是位于成员 Runner 与知识源之间的 Control Plane 服务——唯一决定某个主体可触达哪些知识库的一方。它由一个所有者服务两类受众：管理员读取并维护持久目录，Runner 读取经授权的目录、列出并读取其中的文档，并搜索它。两者都经过这里，因为目录与授权决定必须一致；一个展示了搜索会拒绝的知识库的目录，比二者单独存在更糟。导入它以编写 gateway 提供方，或消费一个。
...
面向成员的方法都不返回部分答案。无法完成授权的目录、在被拒知识库上的列表，以及范围被拒的检索都会抛出，因为对读到它的模型而言，被悄悄收窄的结果与正确结果无法区分。
...
- **每个面向成员的操作都按检索判定** —— 列表和阅读也由 `knowledge.search` 准许，这是产品决策，而不是因为这些操作相同。想把它们分开的部署需要授予 `knowledge.read` 并修改提供方的那些动作常量（[Agent Note](../../../.agents/notes/proposed/feature/2026-09-16-member-knowledge-browsing-and-retrieval.zh.md)）。
- **接缝本身不限定内容读取的规模** —— 调用方给出字节上界，提供方再套上自己的；接缝没有页、区间或偏移量，因此两者都装不下的文档只能以文本读取，或者读不到。
...
**运行时不变量：**不发布伴随文件：该 seam 不拥有注册表，也不发布事件流；被检索的知识库已对检索主体授权这一关系存在于 provider 中，因为授权正是它做出的。
