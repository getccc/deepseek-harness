...
`access/` 组决定某个主体能否对某个资源执行某个操作，持有该决定所读取的角色、授权与受管资源，并记录决定了什么。评估刻意保持很小——默认拒绝，角色的授权准入，多个角色取并集，被禁用的资源一律拒绝——没有显式拒绝、角色继承或表达式语言，因此一次拒绝可以通过列出产生它的授权来解释。两半都是从代码播种的封闭目录：管理员用已注册的权限对组合角色，审计记录只携带目录词与短 token，从不携带任何人的工作内容。
