...
已结算 Token 是各笔结算之和；已预留 Token 是尚无人应答的各笔 Reservation 之和。一个存储的运行总额会成为真相的第二处住所，而两者最终会以只有审计才能发现的方式彼此不一致。`reserve` 先读取预算，只在存在上限时才汇总该周期，因为没有上限时汇总结果无法拒绝请求。
...
- **有上限时，`reserve` 会读取整个周期** —— 余额检查会汇总该周期持有的每个 Reservation，因此受限组织的调用会随周期内记录增多而变慢。[写入路径 Agent Note](../../../.agents/notes/implemented/architecture/2026-09-17-control-plane-sqlite-write-path.zh.md) 记录了测量结果，以及能消除这次扫描的触发器维护总额。
