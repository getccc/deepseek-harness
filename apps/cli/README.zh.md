...
运行命令时所在的目录将作为默认 workspace 根目录。`web`、`headless`、`sdk`、`sdk-minimal` 和 `acp` profile 在首次使用时会从随附模板自动初始化。使用 `--from-default-profile` 可以基于这些模板之一，在尚未使用的非内置名称处创建其他 profile；通过 `dsh plugin` 则可以初始化一个以 base 为基础的 profile。

## 应用参数

单文件可执行文件构建同时也是每个子进程的 Node：插件用自己的 worker 脚本 spawn `process.execPath` 时会重新进入该可执行文件，因此当第一个参数是指向现有 `.js`、`.mjs` 或 `.cjs` 文件的绝对路径时，启动器会完全按 `node <script>` 的方式运行该脚本，绝不把它当作 `dsh` 命令读取。每一次 `dsh` 调用都以选项开头，所以启动器接受的任何用法都不会被遮蔽；普通 Node 启动不受影响。这是在服务一个子进程，不是应用启动器。
