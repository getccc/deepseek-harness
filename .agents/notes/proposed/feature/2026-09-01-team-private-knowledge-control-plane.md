...
[Member browsing and retrieval](2026-09-16-member-knowledge-browsing-and-retrieval.md) owns that second delivery and revises it on three points: documents are read for a member's own panel rather than for the model, the document's knowledge base is verified upstream on every call instead of signed into the reference, and `knowledge.search` admits the read where this note reserved `knowledge.read`. Everything else here still holds.
...
- `GET /api/v1/knowledge-bases`, reading `id`, `name`, `description`, `type`, `knowledge_count`, `processing_count`, `embedding_model_id`, and `updated_at` from each entry. `chunk_count` is not read: it answers zero for knowledge bases whose chunks a search returns, so a count taken from it would tell an administrator that indexed content is absent;
