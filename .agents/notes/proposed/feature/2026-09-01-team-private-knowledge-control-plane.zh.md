...
| `packages/knowledge/knowledge-gateway-http` | 基于 Bearer Token 的面向 Runner HTTP 适配器；拥有知识 `protocolVersion` 并验证每个解码后的请求 |
...
- 在成功完整列表中缺失的条目会被退役：先删除其受治理资源，连同所有指名它的授权，然后删除目录行。目录说的就是数据源说的，管理员读到的是现存的知识库，而不是曾经存在过的历史。因此数据源返回不完整列表会让管理员失去已做出的授权，条目回来时也不带任何授权。
...
`/knowledge` 是注册在 `ctx.commandUi` 上、由浏览器拥有的指令贡献项，在指令列表中与 `/model` 并列。它是一个 `popupMultiSelect`：勾选本身不改变任何东西，一个应用控件让整组勾选一次生效。首行`全部已授权`是 `exclusive` 的——勾选它会清空逐项勾选，勾选任一知识库也会清空它——因此成员无法构造出既表示“今后新授权的一切”又表示“恰好这几个”的集合。不勾选任何一项并应用即`关闭`，这与取消最后一个知识库的勾选是同一个动作。应用会经 `knowledge` 远程追加一条 `knowledge/scope` 事件；它不启动模型轮次。第一阶段的指令不接受查询，因为人工指令不经过模型轮次，而把状态修改与隐藏提示词组合会形成第二条输入路径。

目录中不再存在的已选知识库以不勾选的形式列出并标注为不可用，因此应用时移除它是成员的明确动作，而不是静默缩小。在此之前记录的选择保持不变，指名它的检索会失败。

composer 的左侧区域显示本地化 Chip：`知识库：全部`、`知识库：关闭`或所选名称。Chip 只读取会话投影——它显示的名字就是日志记录下来的那些——并且只报告不动作，因为一个也能改范围的 Chip 会成为写同一条事件的第二条路径。重连和策略修订失效会刷新目录名称与可用性，而不重写已记录的选择。
...
[成员浏览与检索](2026-09-16-member-knowledge-browsing-and-retrieval.zh.md)接管了这个第二阶段，并在三点上做了修订：文档是为成员自己的面板而读，不是为模型而读；文档所属知识库在每次调用时到上游校验，而不是签进引用；准许该阅读的是 `knowledge.search`，而不是本注记预留的 `knowledge.read`。此处其余内容依然成立。
