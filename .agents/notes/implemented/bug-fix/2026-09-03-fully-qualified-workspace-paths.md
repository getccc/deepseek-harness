...
Canonical paths remain the registry identity. A default title uses the final path segment, or `node:path`'s parsed root when that segment is empty. This refines the display rule owned by [same-basename Workspace adoption](../../archived/bug-fix/2026-07-31-same-basename-workspace-adoption.md) without making titles unique.
