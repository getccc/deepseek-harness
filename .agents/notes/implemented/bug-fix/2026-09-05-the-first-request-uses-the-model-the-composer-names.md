...
**Make the client submit the substituted default through `selectModel` before its first prompt.** Rejected: `session.prompt` is the enforcement boundary the [default-follows-the-picker decision](../../archived/feature/2026-08-07-default-model-follows-the-picker.md) names, a client-side pin would race the prompt it precedes, and every other client would need the same pin. It would also save the substitution as the machine's default through `saveSelection`, turning a per-member catalog fact into a machine-wide preference.
