...
**模型目录携带可选的产品分类。** LLM 注册表在提供方元数据中保留 `category: 'built-in'`，`session/modelCatalog` 再把它投影到提供方分组中。浏览器客户端本地化该分类的标题，provider 与 model id 仍是选择标识。普通提供方继续显示适配器持有的名称。
...
共享提供方与目录类型增加一个可选字面量分类。不渲染标题的消费方可以忽略它，展示客户端则无需匹配提供方名称即可本地化内置分组。
