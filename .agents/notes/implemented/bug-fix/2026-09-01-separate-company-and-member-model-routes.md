...
**The model catalog carries an optional product category.** The LLM registry preserves `category: 'built-in'` in provider metadata, and `session/modelCatalog` projects it into the provider group. Browser clients localize the canonical `built-in` route even when that optional metadata is absent, and localize any other group carrying the category. Provider and model ids remain the selection identity. Ordinary providers continue to render the adapter-owned name.
...
The shared provider and catalog types gain one optional literal category. Consumers that do not render headings may ignore it. Presentation clients use the stable `built-in` provider id as the authoritative localization signal and the category for any additional product-owned groups; they never match a provider display name.
