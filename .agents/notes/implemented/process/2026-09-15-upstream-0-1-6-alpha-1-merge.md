...
The Web e2e lane, recorded-session snapshots, and the packaged desktop Runner were not re-run for this merge: upstream moved profile resolution to runtime modes and the packaged carriers onto them, so the WeWork installer build needs its own verification. Migrating the fork's synchronous Session history reads onto projections is deferred work.
