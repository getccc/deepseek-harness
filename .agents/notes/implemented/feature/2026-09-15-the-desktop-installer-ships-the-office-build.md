...
**The installer ships the office skills.** `DSH_TEAM_SKILLS` stages a directory of skill folders as `runner/skills`, and the deployment patch re-enables the host `skill-filesystem` row with `includeDefaultRoots: false` and that directory as its only `customSkillDirs` entry. `dsh-web-app` disables that row because presets own local discovery, so without re-enabling it the staged skills reached no catalog; as a host row it registers into the global skill layer every preset's catalog merges, beside the member's own roots that each preset still discovers. `DSH_TEAM_PPT_SKILL`, `DSH_TEAM_WORD_SKILL`, and `DSH_TEAM_EXCEL_SKILL` name the skill each office kind loads first; packaging fails unless each names a folder inside `DSH_TEAM_SKILLS`. The names are recorded in application metadata as `teamOfficeSkills` and written into the `office` row as `pptSkill`, `wordSkill`, and `excelSkill` beside the staged template path. The deployment refuses office skills without a staged skill directory.
