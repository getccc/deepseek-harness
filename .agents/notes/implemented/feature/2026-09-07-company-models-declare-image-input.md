...
The Welinkin deployment recreates `models.sqlite`, registers its two models again, and marks `Welinkin Model` as accepting images; the packaged administration console gains the checkbox on its next build. Image token estimates on the `built-in` route come from the last listing, so a route that has never listed prices an image as text until the first prepared call.

Unit suites cover the catalog column and version refusal, the endpoint's catalog body, the transport's wire validation, the administration API's default and refusals, and the adapter's remote resolution, inline serialization, refusal, and pricing. No keyless recorded-session snapshot covers the Team transport, because the snapshot harness composes no Control Plane; the image path was verified end to end on the Welinkin deployment from a source-launched Runner.
