...
**A member who may not search is offered no switch.** `POST /team/web/access` answers the same `web.search` decision without running a search, the Team provider's `permitted()` asks it, and `ctx.web.searchPermitted()` reads it from whichever provider a search would use. The chat composition's switch withholds the tools the moment an agent is created, asks that question once, and for a refused or unreachable answer logs nothing and registers no `/web` command, so the composer chip, which reads the `webAccess` Remote's state when a conversation opens, never appears; a member whose grant is revoked later is refused by the Remote on the next open. A permitted member gets the command on the agent's own scope and the log's value.
