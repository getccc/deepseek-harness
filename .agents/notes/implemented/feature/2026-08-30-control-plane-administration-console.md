...
The [browser-application note](../architecture/2026-08-30-administration-console-as-a-browser-application.md) owns the presentation: administration is a React and Ant Design application over a JSON API, while `dsh-team-shell` owns the device-confirmation page alone. Persistent navigation reaches overview, departments, users, roles, menus, and models. A user's expandable row carries that user's device inventory and revoke actions rather than a separate device page.

The administration API registers governed control resources for organization, member, role, device, and model-catalog administration. Password authentication creates an administration session only for an account holding `organization.admin.access`; read and write routes then ask distinct code-registered permissions where the action differs. A deployment bootstraps its first administrator outside the console, then that administrator composes roles from `PERMISSION_CATALOG`, binds and unbinds them on users, provisions account passwords, replaces passwords with account-wide session and device revocation, and manages organization, device, and model records through audited writes.

The role page reads both type and resource grants. Its permission and navigation editors create type grants for organization-wide operations. Its model-resource editor resolves catalog models to managed-resource ids and writes exact `model.discover` and `model.invoke` grants for the selected models, so the Runner's model catalog follows the role without granting every model.
