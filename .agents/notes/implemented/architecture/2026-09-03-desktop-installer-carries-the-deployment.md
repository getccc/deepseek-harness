...
The packaged executable could not run `--profile team` at all before this change. Its dependency closure was missing three workspace peers the Team layers import at runtime — `dsh-knowledge`, `dsh-session-title-llm`, and `dsh-util-workspace-path` — so `knowledge`, `tool-knowledge`, `api-knowledge-controller`, and `session-title-llm` failed to import on every boot. `verify-runtime-closure` exists to catch exactly this and passed, because it read workspace manifests only from `packages/*/*` and `vendor/*`, so it never followed the dependencies of `@deepseek-ai/dsh` in `apps/cli`, through which all three enter the graph. The manifest names them, and [the runtime closure check follows every workspace member](../bug-fix/2026-09-17-the-runtime-closure-check-follows-every-workspace-member.md) records the wider check.
